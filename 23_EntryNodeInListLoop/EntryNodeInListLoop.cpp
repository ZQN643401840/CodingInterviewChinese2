...
ListNode* EntryNodeOfLoop(ListNode* pHead)
{
    ...
    // Just only 2 nodes
    if (p_slow == p_fast)   return p_slow;
    auto senty{p_fast->m_pNext};
    while (true)
    {
        if (p_slow == p_fast)   break;  

        // 迭代更新
        if (nullptr == p_fast->m_pNext || nullptr == p_slow) break;            
        else 
        {
    ...
    if (nullptr == p_fast->m_pNext || nullptr == p_slow) return nullptr;  
